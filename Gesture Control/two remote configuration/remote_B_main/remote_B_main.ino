 ...
 * states and receving commands from the Atom to vibrate a motor. Charge current is set at 630 ma.  
 * Adjust accordingly for the battery capacity (or disable this code at line 138).
 *    
...
const int numStates = 1;       // Set this variable to the number of device states.  In this case, either on or off  
const int thisDeviceID = 3;    // DO NOT USE 0 or 1.  Use 2 for remote A, 3 for remote B, 4 for remote C, etc.
...
void OnDataSent(const uint8_t *mac_addr, esp_now_send_status_t status) {
  if ((status == ESP_NOW_SEND_FAIL)){
    delay(50);
    if (usbVoltage < 2) {  // Put device to sleep if it is not charging
...
void OnDataRecv(const uint8_t * mac_addr, const uint8_t *incomingData, int len) {
  memcpy(&incomingMessage, incomingData, sizeof(myData));   
  //Read incoming data from receiver to set different vibration patterns  
  if (incomingMessage.message == "SELECT") {
    delay(50);
    vibrate(100);    
  }
  else if (incomingMessage.message == "BUZZ") {
    vibrate(100);
  }
  else if (incomingMessage.message == "2XBUZZ") {
    vibrate(100);
    delay(50);
    vibrate(100);
  }
  else if (incomingMessage.message == "SHORTBUZZ") {
    vibrate(30);
  }
  else if (incomingMessage.message == "LONGBUZZ") {
    vibrate(250);
  }
  else if (incomingMessage.message == "INVALID") {
    vibrate(100);
    delay(50);
    vibrate(300);
  }  
...
// Code is run when the big button is pressed
